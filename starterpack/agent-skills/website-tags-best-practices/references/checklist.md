...
7 checks for proper document structure and metadata. For each one: what to scan
for, what passes, and what to flag as a violation.

Scope: template files including `.html`, `.php`, `.njk`, `.liquid`, `.astro`,
`.vue`, `.jsx`/`.tsx` layout/page files that render a full document shell.
