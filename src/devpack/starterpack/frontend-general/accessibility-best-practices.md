...
- `F55`: Scripts that call `.blur()` when an element receives focus
- `F78`: CSS that sets focus indicator to invisible (e.g. same color as background)
...
- `F89`: `<a>` containing only an `<img>` where `alt=""` or `alt` is missing
...
**Flag as violation:**
- `F1`: CSS reordering (flex `order`, `position`) that changes meaning when styles are removed
- `F49`: Layout tables that produce nonsensical reading order when linearized
- Any `tabindex` value > 0
- `F32`/`F33`: Whitespace or `<br>` used for visual spacing instead of semantic structure
...
- `F42`: Emulated links (`<span>` with `onclick`) that don't respond to Enter key
- `F54`: Mouse-only event handlers on custom widgets
